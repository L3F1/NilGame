# Historical host assessment ? superseded

Do not use as current guidance. In particular the union-interior exactness, single skin tolerance, watertightness and remaining-feature claims below are superseded by docs/host-capability-map.md and the current query contracts. Timing figures are historical evidence, not rerun measurements.
