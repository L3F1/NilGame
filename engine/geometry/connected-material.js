...
float materialField(vec4 p,int region){
  ...
  for(int j=0;j<16;j++){
    ...
    for(int k=0;k<6;k++){
      ...
      if(r.x<.5)d=m.x>.5?length(p-n)-m.z:dot(p,n)-m.z;
...
float localAO(vec4 p,vec4 normal,int region){
  ...
  for(int j=0;j<4;j++){
    ...
    if(r.x<.5?length(q)>=r.z:r.w<.5&&q.w<=cs(r.z/r.y))continue;
...
vec3 finishMaterial(vec4 p,vec4 normal,vec4 viewRay,int region,int owner){
  ...
  if(pr.w>.5){
    ...
    vec3 local;
    if(r.x<.5)local=normalize((p-center).xyz);
    else{
      center=-center;
  ...
  float diffuse=max(0.,dot(normal,light)),fill=max(0.,-dot(normal,light));
  vec4 halfRaw=light-viewRay;float spec=length(halfRaw)>1e-5?pow(max(0.,dot(normal,normalize(halfRaw))),48.):0.;
  float fresnel=pow(1.-clamp(-dot(normal,viewRay),0.,1.),4.);
